...
export const getServerSideProps = async ({ query }) => {
    ...
    try {
        const res = await axios.get(`users/confirmation?confirmation_token=${confirmationToken}`)
        console.log(res)
        const user = res.data
        return {
            props: {
                ...
                <div className={inter.className}>
                    ...
                    <div className={sharedStyles.center}>
                        <p className={inter.className}>
                            Your email has been verified. Now you can login with your GitHub account. 
